package com.jd.algorithm.L05BinarySearchTree.java.LOptional04MoreaboutBinarySearch;
...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    for (int i = -1; i <= m + 1; i++) {
      ...
      if (BinarySearch.firstGreaterOrEqual(arr, i) != LinearSearch.firstGreaterOrEqual(arr, i))
        throw new IllegalArgumentException("firstGreaterOrEqual error! " + i);

      if (BinarySearch.firstGreaterThan(arr, i) != LinearSearch.firstGreaterThan(arr, i))
        throw new IllegalArgumentException("firstGreaterThan error! " + i);

      if (BinarySearch.lastLessOrEqual(arr, i) != LinearSearch.lastLessOrEqual(arr, i))
        throw new IllegalArgumentException("lastLessOrEqual error! " + i);

      if (BinarySearch.lastLessThan(arr, i) != LinearSearch.lastLessThan(arr, i))
