package com.jd.algorithm.L05BinarySearchTree.java.LOptional06PredecessorandSuccessorinBST;
...
public class BST<Key extends Comparable<Key>, Value> {
  ...
  public Key predecessor(Key key) {
    ...
    if (node.left != null) return maximum(node.left).key;
  ...
  public Key successor(Key key) {
    ...
    if (node.right != null) return minimum(node.right).key;
  ...
  private boolean contain(Node node, Key key) {
    ...
    else if (key.compareTo(node.key) < 0) return contain(node.left, key);
    else // key > node->key
  ...
  private Node ceil(Node node, Key key) {
    ...
    if (node.key.compareTo(key) < 0) return ceil(node.right, key);
  ...
  Node predecessorFromAncestor(Node node, Key key) {
    ...
    if (node.key.compareTo(key) == 0) return null;
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < N; i++) arr[i] = new Integer(i);
    ...
    for (int i = 0; i < N; i++) bst.insert(new Integer(arr[i]), Integer.toString(arr[i]));
    ...
    for (int i = 0; i < 2 * N; i++) {
      ...
      if (i < N) assert res.equals(Integer.toString(i));
