package com.jd.algorithm.L04Heap.java.L03ShiftUp;
...
public class PrintableMaxHeap extends MaxHeap<Comparable<Integer>> {
  ...
  public void treePrint() {
    ...
    for (int level = 0; level < maxLevel; level++) {
      ...
      for (int indexCurLevel = 0; indexCurLevel < curLevelNumber; index++, indexCurLevel++) {
        line1 =
            putNumberInLine(
                (Integer) data[index], line1, indexCurLevel, curTreeMaxLevelNumber * 3 - 1, isLeft);
        isLeft = !isLeft;
      }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < N; i++) maxHeap.insert(new Integer((int) (Math.random() * M)));
    maxHeap.treePrint();
  }
}
