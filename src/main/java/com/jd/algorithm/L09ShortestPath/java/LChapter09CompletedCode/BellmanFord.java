package com.jd.algorithm.L09ShortestPath.java.LChapter09CompletedCode;
...
public class BellmanFord<Weight extends Number & Comparable> {
  ...
  public BellmanFord(WeightedGraph graph, int s) {
    ...
    for (int i = 0; i < G.V(); i++) from[i] = null;

    // 设置distTo[s] = 0, 并且让from[s]不为NULL, 表示初始s节点可达且距离为0
    distTo[s] = 0.0;
    from[s] =
        new Edge<Weight>(s, s, (Weight) (Number) (0.0)); // 这里我们from[s]的内容是new出来的, 注意要在析构函数里delete掉
    ...
    for (int pass = 1; pass < G.V(); pass++) {
      ...
      for (int i = 0; i < G.V(); i++) {
        ...
        for (Object item : G.adj(i)) {
          ...
          if (from[e.v()] != null
              && (from[e.w()] == null
                  || distTo[e.v()].doubleValue() + e.wt().doubleValue()
  ...
  void showPath(int w) {
    ...
    for (int i = 0; i < res.size(); i++) {
      ...
      if (i == res.size() - 1) System.out.println(res.elementAt(i).w());
    }
  }
  ...
  public static void main(String[] args) {
    ...
    if (bellmanFord.negativeCycle()) System.out.println("The graph contain negative cycle!");
    else
      for (int i = 0; i < V; i++) {
        if (i == s) continue;
        ...
        } else System.out.println("No Path to " + i);
