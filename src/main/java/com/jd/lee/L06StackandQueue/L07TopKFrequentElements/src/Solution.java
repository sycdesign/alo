package com.jd.lee.L06StackandQueue.L07TopKFrequentElements.src;
...
class Solution {
  ...
  private class PairComparator implements Comparator<Pair<Integer, Integer>> {
    ...
    public int compare(Pair<Integer, Integer> p1, Pair<Integer, Integer> p2) {
      if (p1.getKey() != p2.getKey()) return p1.getKey() - p2.getKey();
  ...
  public List<Integer> topKFrequent(int[] nums, int k) {
    ...
    if (k > freq.size())
      throw new IllegalArgumentException(
          "k should be less than the number of unique numbers in nums");

    // 扫描freq,维护当前出现频率最高的k个元素
    // 在优先队列中,按照频率排序,所以数据对是 (频率,元素) 的形式
    PriorityQueue<Pair<Integer, Integer>> pq =
        new PriorityQueue<Pair<Integer, Integer>>(new PairComparator());
    for (Integer num : freq.keySet()) {
      ...
      } else pq.add(new Pair(numFreq, num));
    }
