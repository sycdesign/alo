package com.jd.lee.L02TimeComplexity.L03CommonCodeforTimeComplexity;
...
public class Main {
  ...
  private static void swap(Object[] arr, int i, int j) {
    ...
    if (i < 0 || i >= arr.length) throw new IllegalArgumentException("i is out of bound.");

    if (j < 0 || j >= arr.length) throw new IllegalArgumentException("j is out of bound.");
  ...
  private static int sum(int n) {
    ...
    if (n < 0) throw new IllegalArgumentException("n should be greater or equal to zero.");
  ...
  private static void selectionSort(Comparable[] arr, int n) {
    ...
    for (int i = 0; i < n; i++) {
      int minIndex = i;
      for (int j = i + 1; j < n; j++) if (arr[j].compareTo(arr[minIndex]) < 0) minIndex = j;
  ...
  private static void printInformation(int n) {
    ...
    for (int i = 1; i <= n; i++)
      for (int j = 1; j <= 30; j++) System.out.println("Class " + i + " - " + "No. " + j);
  }
  ...
  private static String intToString(int num) {
    ...
    s.reverse();
    if (sign == "-") return sign + s.toString();
  ...
  private static void hello(int n) {
    ...
    for (int sz = 1; sz < n; sz += sz)
      for (int i = 1; i < n; i++) System.out.println("Hello, Algorithm!");
  }

  // O(sqrt(n)) Time Complexity
  private static boolean isPrime(int num) {

    for (int x = 2; x * x <= num; x++) if (num % x == 0) return false;
    return true;
  }
  ...
  private static boolean isPrime2(int num) {
    ...
    for (int x = 3; x * x <= num; x += 2) if (num % x == 0) return false;
