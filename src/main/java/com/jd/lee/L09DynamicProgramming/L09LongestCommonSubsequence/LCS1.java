package com.jd.lee.L09DynamicProgramming.L09LongestCommonSubsequence;
...
public class LCS1 {
  ...
  private int lcs(String s1, String s2, int m, int n) {
    ...
    int res = 0;
    if (s1.charAt(m) == s2.charAt(n)) res = 1 + lcs(s1, s2, m - 1, n - 1);
    else res = Math.max(lcs(s1, s2, m - 1, n), lcs(s1, s2, m, n - 1));
