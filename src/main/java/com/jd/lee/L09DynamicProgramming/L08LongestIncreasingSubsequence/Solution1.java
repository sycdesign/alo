package com.jd.lee.L09DynamicProgramming.L08LongestIncreasingSubsequence;
...
public class Solution1 {
  ...
  public int lengthOfLIS(int[] nums) {
    ...
    Arrays.fill(memo, -1);
    int res = 1;
    for (int i = 0; i < nums.length; i++) res = Math.max(res, getMaxLength(nums, i));
  ...
  private int getMaxLength(int[] nums, int index) {
    ...
    for (int i = 0; i <= index - 1; i++)
      if (nums[index] > nums[i]) res = Math.max(res, 1 + getMaxLength(nums, i));
