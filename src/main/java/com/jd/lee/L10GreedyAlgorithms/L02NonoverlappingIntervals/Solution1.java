package com.jd.lee.L10GreedyAlgorithms.L02NonoverlappingIntervals;
...
public class Solution1 {
  ...
  public int eraseOverlapIntervals(Interval[] intervals) {
    ...
    for (int i = 1; i < intervals.length; i++)
      // memo[i]
      for (int j = 0; j < i; j++)
        if (intervals[i].start >= intervals[j].end) memo[i] = Math.max(memo[i], 1 + memo[j]);

    int res = 0;
    for (int i = 0; i < memo.length; i++) res = Math.max(res, memo[i]);
  ...
  public static void main(String[] args) {
    ...
    Interval[] interval2 = {new Interval(1, 2), new Interval(1, 2), new Interval(1, 2)};
    System.out.println((new Solution1()).eraseOverlapIntervals(interval2));

    Interval[] interval3 = {new Interval(1, 2), new Interval(2, 3)};
