...
class Solution {
  ...
  public class MaxHeap<E extends Comparable<E>> {
    ...
    private void siftDown(int k) {
      ...
      while (leftChild(k) < data.getSize()) {
        ...
        if (j + 1 < data.getSize() && data.get(j + 1).compareTo(data.get(j)) > 0) j++;
  ...
  public List<Integer> topKFrequent(int[] nums, int k) {
    ...
    for (int key : map.keySet()) {
      if (maxHeap.size() < k) maxHeap.add(new Freq(key, map.get(key)));
      else if (map.get(key) > maxHeap.findMax().freq) maxHeap.replace(new Freq(key, map.get(key)));
    }

    LinkedList<Integer> res = new LinkedList<>();
    while (!maxHeap.isEmpty()) res.add(maxHeap.extractMax().e);
    return res;
  }
