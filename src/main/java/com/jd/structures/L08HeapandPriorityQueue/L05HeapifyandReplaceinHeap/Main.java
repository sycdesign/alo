...
public class Main {
  ...
  private static double testHeap(Integer[] testData, boolean isHeapify) {
    ...
    if (isHeapify) maxHeap = new MaxHeap<>(testData);
    else {
    ...
    for (int i = 0; i < testData.length; i++) arr[i] = maxHeap.extractMax();
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < n; i++) testData1[i] = random.nextInt(Integer.MAX_VALUE);
